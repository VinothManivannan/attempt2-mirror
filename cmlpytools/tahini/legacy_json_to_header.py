 ...
 *
 * WARNING: THIS FILE IS AUTOGENERATED DO NOT MANUALLY EDIT THIS FILE!
 **************************************************************************************************/
...
C_HEADER_ENDIF = \
"""#define ${unique_header_name}_MODULE_COUNT ${num_modules}
#define ${unique_header_name}_CHAIN_COUNT ${num_chains}
#define ${unique_header_name}_ACTUATOR_COUNT ${num_actuators}
...
def _json_actuator_to_cheader(json_data: Any, description: str, header_contents: str) -> str:
    ...
    return header_contents, module_count, chain_count, 1


def _json_chain_to_cheader(json_data: Any, description: str, header_contents: str) -> Tuple[int, int, int]:
    ...
    Returns:
        str: C header text
        int: The number of CAEF modules described in the json_data
        int: The number of CAEF chains described in the json_data
        int: The number of CAEF actuators described in the json_data
    """
    ...
    return header_contents, module_count, 1, 0


def legacy_json_module_to_cheader_DEPRECATED(json_data: Any, description: str, header_contents: str) -> str:
    """Import a legacy json file containing a CAEF 'Module' and convert it into C header text
    ...
    caef_module = json_data["Module"]
    caef_module = caef_module[next(iter(caef_module.keys()))]

    for child in caef_module["children"]:
        if child[1] in json_data["Struct"]:
            # Get the struct's name. It might be defined in the JSON, otherwise just use camel case
            struct_name = _camel_case(child[1])
            if "ctype" in json_data["Struct"][child[1]]:
                struct_name = json_data["Struct"][child[1]]["ctype"]
            header_contents += "struct " + struct_name + " {\n"

            # Parse the struct's child nodes
            struct_contents = json_data["Struct"][child[1]]["children"]
            for struct_member in struct_contents:
                if struct_member[0] == "Reg" or struct_member[0] == "ReservedReg":
                    reg_details = json_data[struct_member[0]][struct_member[1]]
                    if "signed" in reg_details and reg_details["signed"] is True:
                        integer_type = "int16_t "
                    else:
                        integer_type = "uint16_t "

                    number_of_regs = ""
                    if "repeatfor" in reg_details:
                        if "controlspaces" in json_data["Struct"][child[1]]:
                            repeats = json_data["Struct"][child[1]]["controlspaces"][reg_details["repeatfor"]]
                            number_of_regs = "[" + str(len(repeats)) + "]" if len(repeats) > 1 else ""
                        elif "controlindexes" in json_data["Struct"][child[1]]:
                            repeats = json_data["Struct"][child[1]]["controlindexes"][reg_details["repeatfor"]]
                            number_of_regs = "[" + str(repeats) + "]" if repeats > 1 else ""

                    header_contents += "    // @regmap brief: \"" + reg_details["brief"] + "\"\n"
                    header_contents += "    " + integer_type + struct_member[1].lower() + number_of_regs + ";\n"
                else:
                    header_contents += "    FIXME: Embedded struct found\n"
            header_contents += "};\ntypedef struct " + struct_name + " " + struct_name + ";\n\n"

            # Add control constants if they exist for this struct
            if "controlspaces" in json_data["Struct"][child[1]]:
                header_contents += _extract_constants(json_data["Struct"][child[1]]["controlspaces"],
                                                      child[1].upper(),
                                                      const_type="controlspaces")
            elif "controlindexes" in json_data["Struct"][child[1]]:
                header_contents += _extract_constants(json_data["Struct"][child[1]]["controlindexes"],
                                                      child[1].upper(),
                                                      const_type="controlindexes")
...
def _process_input_enum(input_enum: InputEnum) -> str:
    ...
    for i in range(0, num_members):
        ...
        if "Flags" not in input_enum.name:
            prefix = input_enum.name + "_"

        header_text += "    " + prefix + input_enum.enumerators[i].name + " = " + str(input_enum.enumerators[i].value)
        if i < num_members-1:
            header_text += ","
        header_text += "\n"
...
def _json_module_to_c_header(legacy_input_json: InputJson,
                             ...
                             description: str,
                             header_contents: str) -> Tuple[str, int, int, int]:
    ...
    Returns:
        str: C header text
        int: The number of CAEF modules described in the legacy_json_data
        int: The number of CAEF chains described in the legacy_json_data
        int: The number of CAEF actuators described in the legacy_json_data
    """
...
def legacy_json_to_c_header(legacy_path: str) -> str:
    ...
    if "Module" in json_data:
        ...
        header_contents, num_modules, num_chains, num_actuators = _json_module_to_c_header(input_json,
                                                                                           json_data,
                                                                                           caefname,
                                                                                           header_contents)
    if "Chain" in json_data:
        header_contents, num_modules, num_chains, num_actuators = _json_chain_to_cheader(json_data,
                                                                                         caefname,
                                                                                         header_contents)
    if "Actuator" in json_data:
        header_contents, num_modules, num_chains, num_actuators = _json_actuator_to_cheader(json_data,
                                                                                            caefname,
                                                                                            header_contents)
